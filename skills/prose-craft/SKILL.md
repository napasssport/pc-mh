...
**MoneyHub** triggers: рерайт крипто-новости, пост для Telegram-канала MoneyHub, крипто-пост в стиле канала, переписать новость в стиль MoneyHub
→ Read `${CLAUDE_PLUGIN_ROOT}/registers/moneyhub.md` and follow its voice feature description.
...
### Russian-language AI patterns (HARD FAIL, fix silently)

Применять к текстам на русском. Это типовые «умные» ходы ИИ, которых живой автор избегает.

- **Длинное тире «—»** — НИКОГДА. В русском в роли тире использовать короткий дефис «-»
  с пробелами вокруг («рынок льёт - HYPE прёт»). Длинное «—» = ИИ-признак.
- **Разжёвывание + самоназвание вывода:** «вот и весь секрет», «по сути напоминание»,
  «и в этом вся суть», «именно поэтому это важно», «что это значит для нас» как ярлык к
  уже сделанному объяснению. Если мысль уже раскрыта абзацем, не подытоживать её ещё раз.
- **Симметричные/зеркальные пары ради красоты:** «X держат, держится и Y», «было мертво -
  стало живо», хиазмы и параллелизмы, которые звучат сконструированно. Ломать симметрию.
- **Олицетворение-умилизм:** «рынок как будто не в курсе», «биткоин обиделся»,
  «токен решил» - наделение актива сознанием ради остроумия.
- **Морализаторская концовка-обобщение:** финал в виде округлого вывода «для рынка»
  («спрос, который только начинает разгоняться», «потеряно не значит навсегда»), а также
  отдельный финальный абзац «что это значит». Тест: убрать последний абзац - если смысл
  не потерялся, это лишний ИИ-довесок, удалить. Концовка должна быть резкой, конкретной
  или с подколкой, а не нравоучением.
- **Драматизация-перифраз:** пересказ уже изложенного факта «красивым» образом
  («девять лет монеты были мертвы… один человек включил голову, и они ожили»). Факт уже
  сказан - не переигрывать его драматично.
- **Псевдо-вопрос точкой:** «Почему.» вместо «Почему?». Вопрос-крючок ставить с «?».
- **Рубленость:** 3+ очень коротких предложения подряд (пулемёт). Чередовать длину -
  короткое для удара, среднее для связи. Не делать абзац из одних обрубков.
- **Шаблонный одиночный эмодзи в конце КАЖДОГО поста** по одной формуле. Эмодзи - живой
  акцент, а не обязательный хвост; на части постов финал без эмодзи.
