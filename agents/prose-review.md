...
### Russian-language patterns (применять к русскому тексту)

Текст канала — русский. Проверки выше во многом англоязычные; ниже их русские
эквиваленты и добавки. Часть — HARD FAIL (чинить молча), часть — в таблицу.

**HARD FAIL (чинить молча):**
- **Длинное тире «—»** в любом виде. В русском в роли тире — короткий дефис «-» с
  пробелами. Любое «—» заменить на «-».
- **Фатальный «не X, а Y» по-русски:** «это не X, это Y», «не просто X — это Y»,
  «забудьте про X, теперь Y», «не столько X, сколько Y», и фрагментами через точку
  («Не баг. Фича.»). Та же трёхпроходная логика, что для английского.
- **Разжёвывание + ярлык вывода:** «вот и весь секрет», «по сути напоминание», «и в этом
  вся суть», «что это значит для нас» как подпись к уже сделанному объяснению.
- **Морализаторская концовка-абзац:** финальный абзац-обобщение «что это значит для
  рынка» / «спрос только начинает разгоняться» / «потеряно не значит навсегда». Тест:
  убрать последний абзац — если смысл не теряется, это лишний довесок, удалить.
- **Драматизация-перифраз:** пересказ уже изложенного факта «красиво» («девять лет
  монеты были мертвы… и они ожили»).

**В таблицу (advisory):**
- **Олицетворение-умилизм:** «рынок как будто не в курсе», «биткоин обиделся».
- **Симметричные/зеркальные пары** ради красоты («приток держат, держится и цена») —
  это русский случай проверки Conclusion symmetry ниже.
- **Рубленость по-русски:** 3+ очень коротких рубленых предложения подряд — это русский
  случай проверки Monotony/staccato ниже.
- **Псевдо-вопрос точкой:** «Почему.» вместо «Почему?».
- **Шаблонный одиночный эмодзи** в конце КАЖДОГО поста по одной формуле.

Маркеры русского «мёртвого» языка (аналог Dead AI language): «является», «представляет
собой», «играет ключевую роль», «стоит отметить», «важно понимать», «в современном мире»,
«не секрет, что», «таким образом», «в конечном счёте».
...
**6. Conclusion symmetry.** Final 2-3 paragraphs mirroring each other's sentence structure. Flag the structural echo. Human endings are asymmetric. (Русский случай: зеркальные пары «X держат, держится и Y», «было мертво — стало живо».)
...
- This includes staccato (all short declaratives) AND bloat (all long compound sentences). (Русский случай рубленности: 3+ коротких обрубка подряд — частый перекос при имитации «дерзкого» тона.)
